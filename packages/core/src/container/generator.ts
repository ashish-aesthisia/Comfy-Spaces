...
export function generateDockerfile(
  ...
  if (!config.metadata.githubUrl) {
    throw new Error("SpaceConfig.metadata.githubUrl is required");
  }
  if (!config.metadata.pythonVersion) {
    throw new Error("SpaceConfig.metadata.pythonVersion is required");
  }
  if (!config.metadata.spaceId) {
    throw new Error("SpaceConfig.metadata.spaceId is required");
  }
  ...
  if (gpu === "nvidia") {
    ...
    const needsDeadsnakes = pythonMajorMinor !== '3.10';
    ...
    lines.push(`    python${pythonMajorMinor} \\`);
    lines.push(`    python${pythonMajorMinor}-venv \\`);
    lines.push(`    python${pythonMajorMinor}-dev \\`);
    lines.push(`    git \\`);
  ...
  if (releaseTag) {
    lines.push(
      `RUN git clone --branch ${releaseTag} --depth 1 ${githubUrl} ComfyUI`,
    );
  } else if (commitId) {
    if (branch) {
    ...
    lines.push(
      `RUN git clone --branch ${branch} --depth 1 ${githubUrl} ComfyUI`,
    );
  } else {
  ...
  const enabledNodes = config.nodes.filter(
    (node) => !node.disabled && node.githubUrl,
  );
  if (enabledNodes.length > 0) {
    ...
    enabledNodes.forEach((node, index) => {
      ...
      if (node.commitId) {
        if (node.branch) {
          lines.push(
            `RUN git clone --branch ${node.branch} ${nodeUrl} custom_nodes/${nodeName} \\`,
          );
        ...
        lines.push(
          `RUN git clone --branch ${node.branch} --depth 1 ${nodeUrl} custom_nodes/${nodeName}`,
        );
      } else {
        lines.push(
          `RUN git clone --depth 1 ${nodeUrl} custom_nodes/${nodeName}`,
        );
      }
  ...
  if (installManager) {
    lines.push(
      "RUN git clone --depth 1 https://github.com/ashish-aesthisia/ComfyUI-Manager custom_nodes/comfyui-manager",
    );
    lines.push("");
  }
  ...
  if (config.dependencies.length > 0) {
    ...
    const pipCommand =
      gpu === "nvidia" ? `python${pythonMajorMinor} -m pip` : "pip";

    lines.push(`RUN ${pipCommand} install --no-cache-dir --upgrade pip \\`);
    lines.push(
      `    && ${pipCommand} install --no-cache-dir -r requirements.txt`,
    );
    lines.push("");
  }
  ...
  const pythonCmd = gpu === 'nvidia' ? `python${pythonMajorMinor}` : 'python';
  const cmdParts = [pythonCmd, "main.py", ...args];
  const cmdString = cmdParts
    .map((part) => {
...
function generateExtraModelPathsYaml(modelDir: string): string {
  ...
  // Quote the path to handle special YAML characters
  const quotedPath = `"${basePath.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;
  return `comfyui:
  base_path: ${quotedPath}
  is_default: true
